...
class LLMFraudAnalyzer:
    ...
    def __init__(self, api_provider=None, api_key=None):
        """
        Initialize LLM integration
        
        Args:
            api_provider: "openai", "anthropic", "ollama", "gemini", or "huggingface"
            api_key: API key for the service (if required, will use .env if not provided)
        """
        # Use environment variable for default provider if not specified
        self.api_provider = api_provider or os.getenv("DEFAULT_LLM_PROVIDER", "gemini")
        
        # Get API key from parameter, environment variable, or default env key
        if api_key:
            self.api_key = api_key
        else:
            self.api_key = os.getenv(f"{self.api_provider.upper()}_API_KEY")
